...
function isDotOnLine(dot, start, end) {
  // straight line
  if (end[0] - start[0] === 0) return dot[0] === end[0]
  ...
  return Math.abs(slope) <= 1 ? (blockLeftY >= blockBottomY && blockLeftY <= blockTopY) || (blockRightY >= blockBottomY && blockRightY <= blockTopY) : slope > 0 ? !(blockLeftY > blockTopY || blockRightY < blockBottomY) : !(blockRightY > blockTopY || blockLeftY < blockBottomY);
}
...
function isLineDrawn(lineList, startPinIndex, endPinIndex) {
  const lineFound = lineList.find(line => {
    return (startPinIndex === line[0] && endPinIndex === line[1]) || (startPinIndex === line[1] && endPinIndex === line[0]);
  });
